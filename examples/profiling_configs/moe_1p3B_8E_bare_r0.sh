...
nsys profile -s none -t nvtx,cuda -o /workspace/nsys/baseline --force-overwrite true --capture-range=cudaProfilerApi --capture-range-end=stop torchrun $DISTRIBUTED_ARGS /opt/Megatron-LM/pretrain_gpt.py \
    $GPT_ARGS \
    $DATA_ARGS \
    $OUTPUT_ARGS \
